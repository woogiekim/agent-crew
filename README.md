...
- **Memory provider modes** — `~/.agent-crew/bin/memory search` supports `AGENT_CREW_MEMORY_RECALL_MODE=off|legacy|shadow|v2`. `legacy` preserves the existing text result contract, `shadow` runs Recall V2 read-only while returning legacy output, and `v2` preserves `mnemos recall --json` output without direct SQLite/FTS fallback. `AGENT_CREW_MEMORY_FEEDBACK=1` enables provider feedback forwarding, and `AGENT_CREW_MEMORY_STRICT=1` makes provider incompatibility or timeout states fail closed. The stable provider contract is documented in `docs/memory-provider-contract.md`; the legacy direct FTS fallback is deprecated and guarded by compatibility tests.
