...
- **Automatic issue reporting** — installed hooks detect explicit agent-crew bug/error reports or failed `crew` Bash payloads, redact common secrets, deduplicate by fingerprint, and publish a GitHub issue to the agent-crew remote when `gh` is available. Hook script: `core/hooks/auto-issue-report.sh`; contract: `core/rules/auto-issue-reporting.md`.
...
- **Read-only mnemos recall fast path** — `~/.agent-crew/bin/memory search` reads the local mnemos FTS index directly before falling back to the mnemos CLI, then applies a lightweight topic rerank so commercialization/reporting/telemetry recalls prefer actionable findings over adjacent noise. This avoids search-time metadata writes, Obsidian vault git sync, and access-count updates on the prompt-runtime critical path.
