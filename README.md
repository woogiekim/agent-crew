...
- **Bounded stage timeout** — stage-kind defaults bound child lifecycles. Set `AGENT_CREW_STAGE_TIMEOUT_SECONDS` to override every stage, or explicitly set `0` only for deliberate unbounded debugging; adapters without bounded wait plus interrupt support fail before dispatch.
...
- **Memory provider modes** — `~/.agent-crew/bin/memory search` supports `AGENT_CREW_MEMORY_RECALL_MODE=off|legacy|shadow|v2` and defaults to `v2`. `legacy` is an opt-in provider `mnemos search` compatibility path, `shadow` runs Recall V2 read-only while returning legacy output for diagnostics, and `v2` preserves `mnemos recall --json` output without direct SQLite/FTS fallback or synthesized scores. `AGENT_CREW_MEMORY_FEEDBACK=1` enables provider feedback forwarding, and `AGENT_CREW_MEMORY_STRICT=1` makes provider incompatibility or timeout states fail closed. The stable provider contract is documented in `docs/memory-provider-contract.md`.
