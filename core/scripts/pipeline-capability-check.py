...
def planned_dynamic_agents(pipeline: dict[str, Any]) -> tuple[set[str], list[dict[str, Any]]]:
    planned: set[str] = set()
...
def validate_pipeline_capabilities(
    ...
    *,
    ...
    if manifest.get("schema_version") != 1 or not isinstance(agents_manifest, dict):
        return {
            "passed": False,
            "failures": [failure("manifest", None, "invalid_capability_manifest", "manifest must have schema_version=1 and agents object.")],
    ...
    planned_dynamic, dynamic_failures = planned_dynamic_agents(pipeline)
    ...
    for index, stage in enumerate(stages):
        ...
        for agent in agents:
            ...
            if DESTRUCTIVE_CUSTOM_NAME_RE.search(agent):
                failures.append(
                    failure(
                        "stage",
                        index,
                        "custom_agent_name_implies_destructive_authority",
                        "Custom/dynamic agents with destructive names need an explicit manifest-managed role.",
                        agent,
                    )
                )
