#!/usr/bin/env python3
"""Detect agent-crew bug/error signals and publish a deduplicated GitHub issue.
...
def state_dir() -> Path:
    ...
    return home / "state" / "auto-issue-reports"
...
def duplicate_record(record_path: Path, ttl_seconds: int) -> dict[str, Any] | None:
    ...
    if status in {"created", "remote_duplicate", "dry_run"} and time.time() - reported_at <= ttl_seconds:
        return record
    return None
...
def handle(raw: str) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Auto-publish agent-crew bug/error hook reports as GitHub issues.")
    parser.add_argument("--format", choices=("none", "json", "text"), default="none")
