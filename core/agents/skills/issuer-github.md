...
4. Emit the `TARGET_SUMMARY:` block for the dispatcher's Step 0.5:
   ```
   TARGET_SUMMARY:
     backend: github
     ...
     workspace_slug: N/A
   ```
   This block MUST be emitted before any mutating call (label creation, issue
   creation). The dispatcher reads it verbatim to present the confirmation
   screen in Step 0.5.
