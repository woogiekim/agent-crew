...
# internally invokes `review-profile-dispatch.py --agent documenter`
# and writes the framework-computed decision context to
# `${TASK_DIR}/context/capability-skills-documenter.json`. Dispatch alone must not synthesize
...
bash "${CAPABILITY_DISPATCH}" documenter
```

After the helper runs, read the report at `${TASK_DIR}/context/capability-skills-documenter.json`:
- `.matched[] == []` → emit `[crew] CAPABILITY_SKILLS: none agent=documenter` and continue normally (NORMAL state).
