...
```bash
DISPATCH_REPORT="${TASK_DIR}/context/capability-skills-analyst.json"
...
_DISPATCH_LOG="${TASK_DIR}/context/capability-dispatch-analyst.log"
if [ -f "${DISPATCH}" ]; then
  if python3 "${DISPATCH}" \
      --agent analyst \
    ...
    if mv "${_DISPATCH_TMP}" "${DISPATCH_REPORT}" 2>/dev/null; then
      ...
      rm -f "${_DISPATCH_TMP}"
      printf '{"agent":"analyst","matched":[],"fallback":true,"fallback_policy":"generic-analyst-skills"}\n' \
        > "${DISPATCH_REPORT}"
      printf '[crew] DEGRADED | capability-dispatch=mv_failed agent=analyst\n'
    fi
  else
    rm -f "${_DISPATCH_TMP}"
    printf '{"agent":"analyst","matched":[],"fallback":true,"fallback_policy":"generic-analyst-skills"}\n' \
