...
_DISPATCH_LOG="${TASK_DIR}/context/capability-dispatch-resolver.log"
if [ -f "${DISPATCH}" ]; then
  if python3 "${DISPATCH}" \
      --agent resolver \
    ...
    if mv "${_DISPATCH_TMP}" "${DISPATCH_REPORT}" 2>/dev/null; then
      ...
      printf '{"agent":"resolver","matched":[],"fallback":true,"fallback_policy":"generic-resolver-skills"}\n' \
        > "${DISPATCH_REPORT}"
      printf '[crew] DEGRADED | capability-dispatch=mv_failed agent=resolver\n'
    fi
  else
    rm -f "${_DISPATCH_TMP}"
...
- `.matched[] == []` → emit `[crew] CAPABILITY_SKILLS: none agent=resolver` and continue.
