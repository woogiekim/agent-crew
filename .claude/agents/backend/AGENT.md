...
- `{STATE_DIR}/phase.txt` → `DESIGN` (에이전트 내부 단계 전환 — 직접 수정 허용)
...
- `{STATE_DIR}/phase.txt` → `IMPLEMENTATION` (에이전트 내부 단계 전환 — 직접 수정 허용)
...
- `{STATE_DIR}/phase.txt` → `VERIFICATION` (에이전트 내부 단계 전환 — 직접 수정 허용)
...
검증 통과 시: git commit 후 **오직** 아래 이벤트만 emit (pipeline.json 직접 수정 금지):
```bash
echo '{"event":"PHASE_COMPLETE","agent":"backend"}' >> "{STATE_DIR}/events.jsonl"
```

검증 실패 시 (에이전트 내부 재시도 루프 — 직접 수정 허용):
