"""Wave C exemplar (#131) — seed-skill-templates.sh picks up documenter-outline.

Verifies that the existing Wave A seed flow (`core/setup/seed-skill-templates.sh`)
discovers the new Channel B template `documenter-outline.md` and honors
...
`tests/shell/test_skill_templates_seed.bash`; these tests are additive and
prove the user-layer-only invariant (commit 1f89c02) survives for the
documenter dispatcher's Channel B seed.
"""
...
def isolated_templates_dir(tmp_path: Path) -> Path:
    """Copy the real documenter-outline.md into an isolated templates dir."""
    assert SEED_SCRIPT.is_file(), f"seed-skill-templates.sh missing at {SEED_SCRIPT}"
    assert TEMPLATE_SRC.is_file(), (
        f"documenter-outline.md missing at {TEMPLATE_SRC}; "
        "Wave C requires this file to ship."
    )
    src = tmp_path / "templates"
    src.mkdir()
    shutil.copy(TEMPLATE_SRC, src / "documenter-outline.md")
    return src
...
# --------------------------------------------------------------------------- #
# First run: documenter-outline.md must be seeded                             #
# --------------------------------------------------------------------------- #


def test_first_run_seeds_documenter_outline(
    ...
    user_path = empty_user_skills_dir / "documenter-outline.md"
    assert user_path.is_file(), (
        f"documenter-outline.md was not seeded into {empty_user_skills_dir}; "
        f"seed stdout was: {result.stdout!r}"
    )

    assert "seeded user skill: documenter-outline.md" in result.stdout, (
        f"Expected 'seeded user skill: documenter-outline.md' in stdout; "
...
def test_first_run_user_layer_bytes_match_template(
    ...
    src_bytes = (isolated_templates_dir / "documenter-outline.md").read_bytes()
    user_bytes = (empty_user_skills_dir / "documenter-outline.md").read_bytes()
...
def test_second_run_preserves_user_edited_file(
    ...
    user_path = empty_user_skills_dir / "documenter-outline.md"
    custom_content = (
        "# user-edited documenter-outline\n"
    ...
    assert user_path.read_text(encoding="utf-8") == custom_content, (
        "User-layer documenter-outline.md was overwritten — this VIOLATES the "
        "user-layer-only policy from commit 1f89c02."
    )
    assert (
        "user skill already present, template not applied: documenter-outline.md"
        in result.stdout
    )
...
def test_reconcile_advisory_does_not_write_user_layer(
    ...
    assert "documenter-outline" in result.stdout
